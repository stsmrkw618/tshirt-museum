...
export function TodayPick() {
  const [item, setItem] = useState<Tshirt | null>(null);
  ...
  return (
    <section>
      ...
      <Link href={`/collection/${item.id}`} className="group flex gap-4 bg-zinc-900 rounded-xl p-4 hover:bg-zinc-800 transition-colors">
        ...
        <div className="flex flex-col justify-center">
          ...
          {item.character && <p className="text-zinc-500 text-sm">{item.character}</p>}
        </div>
      </Link>
    </section>
  );
